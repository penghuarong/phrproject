import React from 'react'
import styles from './style.less'
import { Tabs, Input, Button, message } from 'antd'
import { login } from '../../services/login'
import { withRouter } from 'dva/router'
const TabPane = Tabs.TabPane

const Yun = (props) => {
  const { phone, password } = props
  const valChange = (e, val) => {
    props.valChange(e, val)
  }
  return (
    <div>
      <Input placeholder="用户名/手机号" value={phone} onChange = {(e) => valChange(e, 'phone')} className={styles.input} />
      <Input placeholder="密码" value={password} onChange = {(e) => valChange(e, 'password')} className={styles.input} />
    </div>
  )
}
const Mesg = (props) => {
  const { userphone, mark } = props
  const valChange = (e, val) => {
    props.valChange(e, val)
  }
  return (
    <div>
      <Input placeholder="手机号" value={userphone} onChange = {(e) => valChange(e, 'userphone')} className={styles.input} />
      <div className={styles.mark}>
        <Input placeholder="验证码" value={mark} onChange = {(e) => valChange(e, 'mark')} className={styles.input} />
        <Button>短信验证码</Button>
      </div>
    </div>
  )
}
@withRouter
class Login extends React.Component {
  constructor () {
    super ()
    this.state = {
      phone: '',
      password: '',
      userphone: '',
      mark: ''
    }
  }
  valChange = (e, val) => {
    this.setState({
      [val]: e.target.value
    })
  }
  login = () => {
    if (this.state.phone !== '') {
      login(this.state.phone, this.state.password).then((res) => {
        message.success('登录成功')
        this.props.history.push('/')
      })
    } else {
      login(this.state.userphone, this.state.mark).then((res) => {
        message.success('登录成功')
        this.props.history.push('/')
      })
    }
  }
  render () {
    const { phone, password, userphone, mark } = this.state
    return (
      <div className={styles.loginBox}>
        <header>
          <div className={styles.top}>
            <img src="data:image/png;base64," alt=""/>
          </div>
        </header>
        <section>
          <div className={styles.login}>
            <div className={styles.left}>
              <img src="http://sso.uc.66yunlian.cn/static/img/backgroud.ed6ed4d.png" alt=""/>
            </div>
            <div className={styles.right}>
              <Tabs defaultActiveKey="1">
                <TabPane tab="云链接账号登录" key="1">
                  <Yun phone={phone} valChange={this.valChange} password = {password} />
                </TabPane>
                <TabPane tab="短信登录" key="2">
                  <Mesg  userphone={userphone} mark={mark} valChange={this.valChange} password = {password} />
                </TabPane>
              </Tabs>
              <Button type="primary" className={styles.loginBtn} onClick={this.login}>登录</Button>
            </div>
          </div>
        </section>
        <footer></footer>
      </div>
    )
  }
}

export default Login
